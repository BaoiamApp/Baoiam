...
const Cards = () => {
  ...
  return (
    <section className="flex flex-col items-center gap-6 px-3 md:gap-8 md:mt-24 md:px-24 md:grid md:items-center md:grid-cols-3">
      {section2.map((info, index) => {
        const cardClass =
          index === 0 ? "card-1" : index === 1 ? "card-2" : "card-3";
        return (
          <div
            key={index}
            className={`group w-full h-[15rem] md:h-[32rem] overflow-hidden relative ${cardClass}`}
          >
            <img
              className="w-full h-full object-cover"
              src={info.img}
