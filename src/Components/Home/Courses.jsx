...
const Courses = () => {
  const data = [
    ...
    },
    {
      ...
      icon: <FaProjectDiagram className="text-indigo-700 text-4xl" />,
    },
    {
      ...
      icon: <FaGlobe className="text-indigo-700 text-4xl" />,
    },
    {
      title: "Expert Mentorship",
      description:
        "Receive guidance from a diverse group of industry professionals who offer valuable insights and support.",
      icon: <FaUsers className="text-indigo-700 text-4xl" />,
    },
  ];

  return (
    <div className="px-10 md:px-20 dark:bg-black pb-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {data.map((stat, index) => (
          <div
            key={index}
            className="bg-zinc-100 bg-gradient-to-r from-teal-50 to-indigo-100 bg-opacity-60 backdrop-blur-sm border border-gray-200 rounded-lg shadow-lg p-6 text-center hover:shadow-xl transition-shadow duration-300 ease-in-out"
          >
            ...
            <p className="text-gray-600 dark:text-black">{stat.description}</p>
          </div>
        ))}
      </div>
    </div>
