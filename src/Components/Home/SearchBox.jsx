...
const SearchBox = () => {
  ...
  return (
    <div className="relative">
      {isExpanded && (
        <div
          className="overlay fixed top-0 right-0 w-full h-full opacity-40 z-20"
      ...
      {!isMobile && (
        <div className="">
          <div
            className={`absolute right-0 -top-3 flex items-center justify-center  rounded-full transition-all duration-500 ease-in-out ${
              isExpanded ? "-top-5 w-60 border border-gray-400" : "w-10"
            } bg-transparent `}
          >
            <RiSearch2Line
              onClick={() => setIsExpanded(true)}
              size={22}
              className={`cursor-pointer mx-auto transition-opacity duration-500 ${
