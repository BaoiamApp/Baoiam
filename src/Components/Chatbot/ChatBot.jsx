...
const ChatBot = () => {
  ...
  const handleSubmit = (event) => {
    ...
    let response;
    if (
      userInputValue.toLowerCase() === "hi" ||
      userInputValue.toLowerCase() === "hello"
    ) {
      response = "Hello! How can I assist you today?";
    } else if (
      userInputValue.toLowerCase() === "Courses List" ||
      userInputValue.toLowerCase() === "courses" ||
      userInputValue.toLowerCase() === "Show me courses" ||
      userInputValue.toLowerCase() === "School Courses"
    ) {
      ...
      {chatOpen && (
        <div className="fixed bottom-24 right-2 md:right-8 w-72 md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white dark:bg-[#15104d] rounded-2xl shadow-md">
          ...
          <form
            ...
            className="flex items-center border border-black dark:border-white rounded-full overflow-hidden mt-4"
          >
            <input
              type="text"
              name="userInput"
            ...
            <button
              type="submit"
              className="bg-indigo-600 hover:bg-indigo-400 text-white font-bold py-1 px-4 rounded-full"
            >
              Send
            </button>
          </form>
